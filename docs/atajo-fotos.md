...
## 2. Dos caminos, y los dos hacen falta

|  | Atajo (este documento) | Cable + `tools/importar_fotos.py` |
|---|---|---|
| Cuándo | Todos los días, solo | Una vez, para el volcado gordo |
| Para qué | Mantener el mapa vivo durante el viaje | Meter años anteriores de golpe |
| Coste | Cero: no tocas nada | Conectar el móvil y un comando |
| Velocidad | Lento por foto (bucle en el móvil) | Mil fotos en segundos |

Empieza por el cable para tener el mapa lleno hoy, y monta el atajo para que se
mantenga solo a partir de mañana.
...
### Por qué mandar los últimos días y no solo los de hoy

Es la decisión 23 otra vez. Como la deduplicación va por **nombre de archivo**,
reenviar una foto que ya estaba no cuesta nada: el servidor responde
`duplicados` y no la guarda dos veces. Así que el atajo manda **las fotos de
los últimos 3 días** en cada envío, y con eso:

- Un día sin cobertura no pierde nada: al siguiente van las de los dos días.
- No hay ninguna cola que mantener en el móvil, ni estado que sincronizar.

En régimen normal, la mayoría de cada envío saldrá como `duplicados`. **Eso no
es ruido: es la señal de que la ventana está funcionando.**
...
### Bloque A — Buscar las fotos

1. **Buscar fotos**
   - Filtro: *Fecha de captura* — *está en los últimos* — **3 días**
   - Ordenar por *Fecha de captura*, ascendente
   - **Límite: 300 elementos.** No es capricho: sin límite, un día de fiesta con
     600 fotos convierte el bucle en varios minutos, y el cuerpo de la petición
     se pasaría de los 128 KB que acepta el servidor. Si te pasas, llegará un
     413 y no se guardará nada.
